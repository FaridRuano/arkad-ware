...
export default function page() {
  ...
  return (
    <>
      ...
      <div className={m('admin__dashboard appointments__page')}>

        {/* 1) Filtros */}
        <section className={m('appointments__filters')}>
          <header className={m('appointments__filtersHeader')}>
            <h1 className={m('appointments__title')}>Citas</h1>

            <div className={m('appointments__filtersActions')}>
              {/* luego: actualizar / limpiar */}
              <button className={m('__button appointments__btnRefresh')} onClick={fetchAppointments} disabled={loading}>
                {loading ? 'Cargando…' : 'Actualizar'}
              </button>
              <button
                className={m('__button secondary appointments__btnClear')}
                onClick={() => {
                  setQ('')
                  setStatus('all')
                  setFrom('')
          ...
          <div className={m('appointments__filtersGrid')}>
            {/* Búsqueda */}
            <div className={m('field appointments__field appointments__fieldSearch')}>
            ...
            {/* Estado */}
            <div className={m('field appointments__field appointments__fieldStatus')}>
            ...
            {/* Desde */}
            <div className={m('field appointments__field appointments__fieldFrom')}>
              <label className={m('field__label')}>Desde</label>
              <input
                className={m('field__input')}
                type="date"
                value={from}
            ...
            {/* Hasta */}
            <div className={m('field appointments__field appointments__fieldTo')}>
          ...
          <div className={m('appointments__quickFilters')}>
            <button
              className={m('chip appointments__chip', quick === 'hoy' && 'active')}
            ...
            <button
              className={m('chip appointments__chip', quick === 'semana' && 'active')}
            ...
            <button
              className={m('chip appointments__chip', quick === 'mes' && 'active')}
        ...
        {/* 2) Tabla */}
        <section className={m('appointments__tableSection')}>
          <header className={m('appointments__tableHeader')}>
            <h2 className={m('appointments__subtitle')}>Resultados</h2>

            <div className={m('appointments__tableMeta')}>
              {/* luego: "Mostrando 1–10 de 120" */}
              <span className={m('appointments__count')}>—</span>
            </div>
          </header>

          <div className={m('appointments__tableWrap')}>
            <table className={m('appointments__table')}>
              <thead className={m('appointments__thead')}>
                <tr className={m('appointments__tr appointments__trHead')}>
                  <th className={m('appointments__th')}>Cliente</th>
                  <th className={m('appointments__th')}>Teléfono</th>
                  <th className={m('appointments__th')}>Fecha</th>
                  <th className={m('appointments__th')}>Duración</th>
                  <th className={m('appointments__th')}>Precio</th>
                  <th className={m('appointments__th')}>Estado</th>
                  <th className={m('appointments__th appointments__thActions')}>Acciones</th>
                </tr>
              </thead>

              <tbody className={m('appointments__tbody')}>
                {loading && (
                  <tr className={m('appointments__tr')}>
                    <td className={m('appointments__td appointments__tdState')} colSpan={7}>
                      <div className={m('appointments__state appointments__stateLoading')}>
                        <span className={m('appointments__stateTitle')}>Cargando citas…</span>
                        <span className={m('appointments__stateText')}>
                ...
                {!loading && error && (
                  <tr className={m('appointments__tr')}>
                    <td className={m('appointments__td appointments__tdState')} colSpan={7}>
                      <div className={m('appointments__state appointments__stateError')}>
                        <span className={m('appointments__stateTitle')}>Ocurrió un error</span>
                      </div>
                    </td>
                  </tr>
                )}
                ...
                {!loading && !error && rows.length === 0 && (
                  <tr className={m('appointments__tr appointments__trEmpty')}>
                    <td className={m('appointments__td appointments__tdState')} colSpan={7}>
                      <div className={m('appointments__state appointments__stateEmpty')}>
                        <span className={m('appointments__stateTitle')}>
                        ...
                        <button
                          className={m('__button secondary appointments__stateAction')}
                          onClick={() => {
                            setQ('')
                            setStatus('all')
                            setFrom('')
                ...
                {!loading && !error && rows.length > 0 &&
                  rows.map((a) => {
                    ...
                    return (
                      <tr className={m('appointments__tr')} key={a?.id ?? a?._id}>
                        <td className={m('appointments__td')}>{a?.name ?? '—'}</td>
                        <td className={m('appointments__td')}>{a?.phone ?? '—'}</td>
                        <td className={m('appointments__td')}>
                        ...
                        {/* ✅ ACCIONES CON MODAL */}
                        <td className={m('appointments__td appointments__tdActions')}>
                          <div className={m('appointments__actions')}>
                            {canCancel(st) && (
                              <button
                                className={m('actionBtn actionBtn--cancel')}
                                title="Cancelar cita"
                            ...
                            {canNoAssistance(st) && (
                              <button
                                className={m('actionBtn actionBtn--noassist')}
        ...
        {/* 3) Paginación */}
        <section className={m('appointments__pagination')}>
          {(() => {
            const limit = 12
            ...
            return (
              <>
                <div className={m('appointments__paginationLeft')}>
                  <span className={m('appointments__pageInfo')}>
                    Página <b>{safePage}</b> de <b>{totalPages}</b>
                  </span>
                </div>

                <div className={m('appointments__paginationRight')}>
                  <button
                    className={m('appointments__navBtn', !canPrev && 'is-disabled')}
                  ...
                  <div className={m('appointments__pageDots')} aria-label="Indicador de páginas">
                    {totalPages === 1 ? (
                      <span className={m('pageMark pageMark--square is-active')} />
                    ) : (
                      indicators.map((it) => (
                        <span
                          key={it.p}
                  ...
                  <button
                    className={m('appointments__navBtn', !canNext && 'is-disabled')}
