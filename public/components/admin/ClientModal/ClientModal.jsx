...
export default function ClientModal({
    ...
    function onCedulaChange(e) {
        ...
        const res = validateClientDocument(value)
        setErrorCedula(res.ok ? "" : res.message)
    }
    ...
    const validate = () => {
        ...
        if (!cedula) return "El documento es requerido"
        const cedRes = validateClientDocument(cedula)
        if (!cedRes.ok) return cedRes.message
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            cedula: validateClientDocument(form.cedula).value,
        ...
        >
            <div className={m('modalCard')}>
                <header className={m('modalHeader')}>
                    <h3 className={m('modalTitle')}>{isEdit ? "Editar cliente" : "Nuevo cliente"}</h3>
                ...
                <form className={m('modalBody')} onSubmit={handleSubmit}>
                    ...
                    <div className={m('modalGrid')}>
                        <div className={m('field')}>
                            ...
                            />
                            {!!errorCedula && <div className={m('field__hint')} style={{ color: "var(--negative)" }}>{errorCedula}</div>}
                        </div>
                        ...
                        <div className={m('field')}>
                            <label className={m('field__label')}>Nombres</label>
                            <input
                                className={m('field__input')}
                        ...
                        <div className={m('field')}>
                            <label className={m('field__label')}>Apellidos</label>
                            <input
                                className={m('field__input')}
                                value={form.lastName}
                        ...
                        <div className={m('field')} style={{ gridColumn: "1 / -1" }}>
                            <label className={m('field__label')}>Email</label>
                            <input
                                className={m('field__input')}
                                value={form.email}
                        ...
                        <div className={m('field')} style={{ gridColumn: "1 / -1" }}>
                            <label className={m('field__label')}>Dirección</label>
                            <input
                                className={m('field__input')}
                                value={form.address}
