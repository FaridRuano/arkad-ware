...
import styles from './ScheduleDateNavigator.module.scss';
...
export default function ScheduleDateNavigator({
    ...
    return (
        <section className={m('schedule-date-navigator')}>
            <div className={m('schedule-date-navigator__top')}>
                <div className={m('schedule-date-navigator__views')}>
                    {VIEW_OPTIONS.map((option) => (
                        <button
                            key={option.key}
                            type="button"
                            className={m(
                                'schedule-date-navigator__view-btn',
                                activeView === option.key && 'is-active',
                            )}
                            onClick={() => setActiveView(option.key)}
                        >
                            <span className={m('schedule-date-navigator__view-label')}>
                                {option.label}
                            </span>
                            <span className={m('schedule-date-navigator__view-value')}>
                                {summary[option.key]}
                            </span>
                        </button>
                    ))}
                ...
                <div className={m('schedule-date-navigator__actions')}>
                    <button
                        type="button"
                        className={m('schedule-date-navigator__arrow')}
                        onClick={() => moveDate(-1)}
                        aria-label="Anterior"
                    >
                        ‹
                    ...
                    <button
                        type="button"
                        className={m('schedule-date-navigator__today')}
                        onClick={handleToday}
                    >
                        Hoy
                    </button>
            ...
            <div className={m('schedule-date-navigator__week')}>
                {weekDays.map((day) => {
                    ...
                    return (
                        <button
                            ...
                            type="button"
                            className={m(
                                'schedule-date-navigator__day',
                                isSelected && 'is-selected',
                            )}
                            onClick={() => onChangeDate(day.iso)}
                        >
                            <span className={m('schedule-date-navigator__day-weekday')}>
                                {day.weekdayLabel}
                            </span>
                            <span className={m('schedule-date-navigator__day-number')}>
                                {day.dayNumber}
                            </span>
                        </button>
                    );
