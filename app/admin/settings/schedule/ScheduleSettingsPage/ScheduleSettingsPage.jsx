...
import styles from './ScheduleSettingsPage.module.scss'
...
const SchedulePage = () => {
  ...
  async function fetchBarberSchedules() {
    ...
    setActiveBarberId((prev) => {
      if (prev && normalized.some((item) => (item?.barber?.id || item?.barber?._id) === prev)) {
        return prev
      }
  ...
  const toggleBarberCard = (barberId) => {
    setActiveBarberId((prev) => (prev === barberId ? null : barberId))
  }
  ...
  return (
    <>
      ...
      <div className={m('schedule-page page')}>
        <section className={m('schedule-page__header')}>
          <div className={m('schedule-page__headerContent')}>
            <div className={m('schedule-page__titleWrap')}>
              <h1 className={m('schedule-page__title')}>Horarios</h1>
              <p className={m('schedule-page__subtitle')}>
        ...
        {loading ? (
          <section className={m('schedule-page__state')}>
            <PageLoader
              title="Cargando horarios..."
              text="Espera un momento mientras se prepara la configuración."
            />
          </section>
        ) : error ? (
          <section className={m('schedule-page__state')}>
            <span className={m('schedule-page__stateTitle')}>Ocurrió un error</span>
            <span className={m('schedule-page__stateText')}>{error}</span>
            <button className={m('__button secondary')} onClick={fetchAll}>
              Reintentar
            </button>
          </section>
        ) : (
          <>
            <section className={m('schedule-page__business')}>
              <div className={m('schedule-business-card')}>
                <div className={m('schedule-business-card__top')}>
                  <div className={m('schedule-business-card__titleWrap')}>
                    <div className={m('schedule-business-card__eyebrow')}>
                      <BriefcaseBusiness size={15} />
                      <span>Configuración general</span>
                    </div>

                    <h2 className={m('schedule-business-card__title')}>Horario general del negocio</h2>
                    <p className={m('schedule-business-card__text')}>
                      Define la base sobre la que luego se organizan los horarios individuales.
                    </p>
                  </div>

                  <div className={m('schedule-business-card__actionsWrap')}>
                    <div className={m('schedule-business-card__actionsMeta')}>
                      <span className={m('schedule-business-card__actionsLabel')}>
                        Acciones generales
                      </span>
                      <span className={m('schedule-business-card__actionsHint')}>
                        Guarda o revierte los cambios del horario principal.
                      </span>
                    </div>

                    <div className={m('schedule-business-card__actions')}>
                      <button
                        className={m('__button schedule-business-card__discardBtn')}
                      ...
                      <button
                        className={m('__button primary schedule-business-card__saveBtn')}
                        onClick={requestSaveBusiness}
                        disabled={businessSaving || !businessHasChanges}
                        type="button"
                      >
                        {businessSaving ? 'Guardando…' : 'Guardar cambios'}
                      </button>
                    </div>
                  </div>
                </div>

                <div className={m('schedule-business-card__grid')}>
                  <div className={m('schedule-business-card__ranges')}>
                    <div className={m('schedule-range-card')}>
                      <div className={m('schedule-range-card__head')}>
                        <h3 className={m('schedule-range-card__title')}>Lunes a viernes</h3>
                        <label className={m('schedule-toggle')}>
                          <input
                            className={m('schedule-toggle__input')}
                            type="checkbox"
                          ...
                          />
                          <span className={m('schedule-toggle__text')}>
                            {businessForm.generalSchedule.weekdays.enabled ? 'Activo' : 'Cerrado'}
                          </span>
                        </label>
                      </div>

                      <div className={m('schedule-range-card__times')}>
                        <div className={m('field schedule-field')}>
                          <label className={m('field__label schedule-field__label')}>Inicio</label>
                          <input
                            className={m('field__input schedule-field__input')}
                            type="time"
                        ...
                        <div className={m('field schedule-field')}>
                          <label className={m('field__label schedule-field__label')}>Fin</label>
                          <input
                            className={m('field__input schedule-field__input')}
                            type="time"
                    ...
                    <div className={m('schedule-range-card')}>
                      <div className={m('schedule-range-card__head')}>
                        <h3 className={m('schedule-range-card__title')}>Sábado</h3>
                        <label className={m('schedule-toggle')}>
                          <input
                            className={m('schedule-toggle__input')}
                            type="checkbox"
                          ...
                          />
                          <span className={m('schedule-toggle__text')}>
                            {businessForm.generalSchedule.saturday.enabled ? 'Activo' : 'Cerrado'}
                          </span>
                        </label>
                      </div>
                    ...
                    <div className={m('schedule-range-card')}>
                      <div className={m('schedule-range-card__head')}>
                        <h3 className={m('schedule-range-card__title')}>Domingo</h3>
                        <label className={m('schedule-toggle')}>
                          <input
                            className={m('schedule-toggle__input')}
                            type="checkbox"
                          ...
                          />
                          <span className={m('schedule-toggle__text')}>
                            {businessForm.generalSchedule.sunday.enabled ? 'Activo' : 'Cerrado'}
                          </span>
                        </label>
                      </div>
                  ...
                  <div className={m('schedule-business-card__summary')}>
                    <div className={m('schedule-summary-chip')}>
                      <span className={m('schedule-summary-chip__label')}>Lun - Vie</span>
                      <span className={m('schedule-summary-chip__value')}>{businessSummary.weekdays}</span>
                    </div>

                    <div className={m('schedule-summary-chip')}>
                      <span className={m('schedule-summary-chip__label')}>Sábado</span>
                      <span className={m('schedule-summary-chip__value')}>{businessSummary.saturday}</span>
                    </div>

                    <div className={m('schedule-summary-chip')}>
                      <span className={m('schedule-summary-chip__label')}>Domingo</span>
                      <span className={m('schedule-summary-chip__value')}>{businessSummary.sunday}</span>
                    </div>
                  </div>

                  <div className={m('schedule-business-card__rules')}>
                    <div className={m('field schedule-field')}>
                      <label className={m('field__label schedule-field__label')}>Intervalo de agenda (min)</label>
                      <input
                        className={m('field__input schedule-field__input')}
                        type="number"
                        min="5"
                    ...
                    <div className={m('field schedule-field')}>
                      <label className={m('field__label schedule-field__label')}>Anticipación mínima (min)</label>
                      <input
                        className={m('field__input schedule-field__input')}
                        type="number"
                        min="0"
                    ...
                    <div className={m('field schedule-field')}>
                      <label className={m('field__label schedule-field__label')}>Máximo días de anticipación</label>
                      <input
                        className={m('field__input schedule-field__input')}
                        type="number"
                        min="1"
            ...
            <section className={m('schedule-page__barbers')}>
              <div className={m('schedule-page__sectionHead')}>
                <h2 className={m('schedule-page__sectionTitle')}>Horarios por barbero</h2>
                <p className={m('schedule-page__sectionText')}>
                  Ajusta la jornada semanal individual de cada miembro del equipo.
                </p>
              </div>

              <div className={m('schedule-page__barberList')}>
                {barberSchedules.map((schedule, index) => {
                  ...
                  const isOpen = activeBarberId === barberId

                  return (
                    <article
                      key={barberId}
                      className={m('barber-schedule-card', !barberActive && 'is-inactive', isOpen && 'is-open')}
                      style={{ '--card-index': index }}
                    >
                      <button
                        type="button"
                        className={m('barber-schedule-card__trigger')}
                        onClick={() => toggleBarberCard(barberId)}
                        aria-expanded={isOpen}
                        aria-controls={`schedule-panel-${barberId}`}
                      >
                        <div className={m('barber-schedule-card__top')}>
                          <div className={m('barber-schedule-card__identity')}>
                            <div className={m('barber-schedule-card__titleRow')}>
                              <span
                                className={m('barber-schedule-card__color')}
                                style={{ background: barberColor }}
                              />
                              <h3 className={m('barber-schedule-card__name')}>{barberName}</h3>
                              {!barberActive && (
                                <span className={m('barber-schedule-card__badge')}>Inactivo</span>
                              )}
                              {!schedule?.hasCustomSchedule && (
                                <span className={m('barber-schedule-card__badge barber-schedule-card__badge--ghost')}>
                                  Nuevo horario
                                </span>
                              )}
                              {hasChanges && (
                                <span className={m('barber-schedule-card__badge barber-schedule-card__badge--accent')}>
                                  Cambios sin guardar
                                </span>
                              )}
                            </div>

                            <p className={m('barber-schedule-card__text')}>
                              Configura los días y horas en que este barbero puede recibir reservas.
                            </p>

                            <div className={m('barber-schedule-card__summary')}>
                              <span className={m('barber-schedule-card__summaryItem')}>
                                {schedule.useBusinessHoursAsFallback ? 'Usa horario general' : 'Horario personalizado'}
                              </span>
                              <span className={m('barber-schedule-card__summaryItem')}>
                                {schedule.weekSchedule.filter((day) => day.enabled).length} días activos
                              </span>
                            </div>
                          </div>

                          <div className={m('barber-schedule-card__triggerMeta')}>
                            {schedule.useBusinessHoursAsFallback && (
                              <div className={m('schedule-inherited-banner')}>
                                Usa el horario general
                              </div>
                            )}
                            <span
                              className={m('barber-schedule-card__chevron', isOpen && 'is-open')}
                              aria-hidden="true"
                            >
                              <ChevronDown size={18} />
                            </span>
                      ...
                      <div
                        id={`schedule-panel-${barberId}`}
                        className={m('barber-schedule-card__panel', isOpen && 'is-open')}
                      >
                        <div className={m('barber-schedule-card__panelInner')}>
                          <div className={m('barber-schedule-card__actionsWrap')}>
                            <div className={m('barber-schedule-card__actionsMeta')}>
                              <span className={m('barber-schedule-card__actionsLabel')}>
                                Acciones del barbero
                              </span>
                              <span className={m('barber-schedule-card__actionsHint')}>
                                Aplica o revierte solo este horario.
                              </span>
                            </div>

                            <div className={m('barber-schedule-card__actions')}>
                              <button
                                className={m('__button barber-schedule-card__discardBtn')}
                              ...
                              <button
                                className={m('__button primary barber-schedule-card__saveBtn')}
                          ...
                          <div className={m('barber-schedule-card__controls')}>
                            <div className={m('schedule-setting')}>
                              <span className={m('schedule-setting__label')}>
                                Referencia de horario
                              </span>

                              <label className={m('schedule-toggle schedule-toggle--stacked')}>
                                <input
                                  className={m('schedule-toggle__input')}
                                  type="checkbox"
                                ...
                                />
                                <span className={m('schedule-toggle__text')}>
                                  Usar horario general como referencia
                                </span>
                              </label>
                            </div>
                          </div>

                          <div className={m('barber-schedule-card__days')}>
                            {schedule.weekSchedule.map((dayItem, dayIndex) => {
                              const isLockedByBusiness = Boolean(schedule?.useBusinessHoursAsFallback)

                              return (
                                <div
                                  className={m('schedule-day-row', isLockedByBusiness && 'is-locked')}
                                  key={`${barberId}-${dayItem.day}`}
                                >
                                  <div className={m('schedule-day-row__left')}>
                                    <div className={m('schedule-day-row__dayWrap')}>
                                      <span className={m('schedule-day-row__day')}>
                                      ...
                                      <label className={m('schedule-toggle')}>
                                        ...
                                        />
                                        <span className={m('schedule-toggle__text')}>
                                          {dayItem.enabled ? 'Activo' : 'Libre'}
                                        </span>
                                      </label>
                                    </div>
                                  </div>

                                  <div className={m('schedule-day-row__times')}>
                                    <div className={m('field schedule-field')}>
                                      <label className={m('field__label schedule-field__label')}>Inicio</label>
                                      <input
                                        className={m('field__input schedule-field__input')}
                                    ...
                                    <div className={m('field schedule-field')}>
                                      <label className={m('field__label schedule-field__label')}>Descanso inicio</label>
                                      <input
                                        className={m('field__input schedule-field__input')}
                                        type="time"
                                    ...
                                    <div className={m('field schedule-field')}>
                                      <label className={m('field__label schedule-field__label')}>Descanso fin</label>
                                      <input
                                        className={m('field__input schedule-field__input')}
                                        type="time"
                          ...
                          <div className={m('barber-schedule-card__footer')}>
                            <div className={m('field schedule-field')} style={{ width: '100%' }}>
                              <label className={m('field__label schedule-field__label')}>Notas</label>
                              <input
                                className={m('field__input schedule-field__input')}
